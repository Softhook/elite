...
/**
 * Initialize the global gamepad instance with game-specific bindings.
 * Called once during setup.
 * 
 * CONTROL SCHEME:
 * ┌─────────────────────────────────────────────────────────┐
 * │ FLIGHT CONTROLS (continuous - polled via state)         │
 * │   Left Stick Y    → Thrust forward/reverse (W/S)       │
 * │   Left Stick X    → Strafe left/right (A/D)            │
 * │   Right Stick X   → Rotate left/right (Q/E)            │
 * │   R2 (analog)     → Thrust forward (W) - analog        │
 * │   L2 (analog)     → Thrust reverse (S) - analog        │
 * │                                                        │
 * │ COMBAT (keyboard bridge)                               │
 * │   A button        → Fire (Space)                       │
 * │   R1              → Fire (Space)                       │
 * │   X button        → Speed boost (R key)                │
 * │   Y button        → Cloak (C key)                      │
 * │   B button        → Surface descent (X key)            │
 * │                                                        │
 * │ NAVIGATION (keyboard bridge)                           │
 * │   D-Pad Up        → Arrow Up (menu nav / missions)     │
 * │   D-Pad Down      → Arrow Down                         │
 * │   D-Pad Left      → Arrow Left (detail screens)        │
 * │   D-Pad Right     → Arrow Right                        │
 * │   Start           → Map (M key)                        │
 * │   Select          → Inventory (I key)                   │
 * │   L1              → Autopilot cycle planet (H key)     │
 * │   L3 (stick click)→ Autopilot cycle service (J key)    │
 * │   R3              → Mission overlay (N key)            │
 * │                                                        │
 * │ MENUS (keyboard bridge)                                │
 * │   A button        → Confirm / Click (Enter)            │
 * │   B button        → Back (Escape)                      │
 * │   L1/R1           → Weapon switch prev/next (,/.)      │
 * └─────────────────────────────────────────────────────────┘
 */
function initGamepad() {
  const gp = new GamepadManager({ toast: true, deadzone: 0.15 });

  // ── Combat ──
  gp.bindKey('a', 'Space');              // A = fire
  gp.bindKey('r1', 'Space');             // R1 = fire (alt)
  gp.bindKey('x', 'KeyR');              // X = speed boost
  gp.bindKey('y', 'KeyC');              // Y = cloak
  gp.bindKey('b', 'KeyX');              // B = surface descent (X key)

  // ── Navigation ──
  gp.bindKey('dpad.up', 'ArrowUp');
  gp.bindKey('dpad.down', 'ArrowDown');
  gp.bindKey('dpad.left', 'ArrowLeft');
  gp.bindKey('dpad.right', 'ArrowRight');
  gp.bindKey('start', 'KeyM');          // Start = galaxy map
  gp.bindKey('sel', 'KeyI');            // Select = inventory
  gp.bindKey('l1', 'KeyH');             // L1 = autopilot planet cycle
  gp.bindKey('l3', 'KeyJ');             // L3 = autopilot service cycle
  gp.bindKey('r3', 'KeyN');             // R3 = mission overlay

  // ── Surface altitude ──
  gp.bindKey('dpad.up', 'KeyZ');         // D-Up also = altitude up (Z key) in surface
  gp.bindKey('dpad.down', 'KeyX');       // D-Down also = altitude down (X key) in surface

  // ── Weapon switching ──
  gp.bindKey('pr', 'Period');            // Paddle Right = minimap zoom in
  gp.bindKey('pl', 'Comma');             // Paddle Left = minimap zoom out

  // ── Menu confirm/back ──
  // A also sends Enter for menu confirmations
  // B also sends Escape for menu back

  window._gamepadManager = gp;
  return gp;
}